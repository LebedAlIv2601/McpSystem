...
from rag_engine import RAGEngine, OllamaError
...
async def ensure_index_built() -> bool:
    ...
    try:
        ...
        logger.error(f"Ollama error while building index: {e}")
        raise
    except Exception as e:
...
async def handle_rag_query(arguments: dict) -> list[TextContent]:
    ...
    try:
        ...
        return [TextContent(type="text", text=json.dumps({
            "error": f"Ollama not available: {e}. Please ensure Ollama is running with nomic-embed-text model."
        }))]
